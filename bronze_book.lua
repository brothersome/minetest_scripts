if not init then
	...
	empty_made = function()
		...
		if check_inventory.right_down(material,'in') then
			take.right_down(material,'in')
			return true
		else
			stepval = stepval / 2
			if stepval < 1 then
	...
	if checkingcnt > 0 then
		...
		if stepval <= 0 then
			if check_inventory.self('default:bronze_ingot 100','main') then
				-- self.label('bronze')
				insert.left_down('default:bronze_ingot 100', 'in')
				activate.left_down(2)
				take.left_down('alchemy:essence_high 2', 'out')
			else
				if not check_inventory.self('default:tin_ingot 99','main') then
					-- self.label('tin')
					if not check_inventory.right_down('alchemy:essence_high', 'out') and check_inventory.self('alchemy:essence_high 4', 'main') then
						insert.right_down('alchemy:essence_high 2', 'out')
					else
						if not check_inventory.right_down('alchemy:essence_high', 'out') and check_inventory.self('alchemy:essence_high 2', 'main') then
							insert.right_down('alchemy:essence_high', 'out')
						end
					end
					insert.right_down('default:tin_ingot','in')
					...
					if not check_inventory.self('default:copper_ingot 99','main') then
						-- self.label('copper')
						if not check_inventory.right_down('alchemy:essence_high', 'out') and check_inventory.self('alchemy:essence_high 10', 'main')  then
							insert.right_down('alchemy:essence_high 10', 'out')
						else
							if not check_inventory.right_down('alchemy:essence_high', 'out') and check_inventory.self('alchemy:essence_high 2', 'main')  then
								insert.right_down('alchemy:essence_high','out')
							end
						end
						insert.right_down('default:copper_ingot','in')
						activate.right_down(2)
						material_empty = 'default:copper_ingot'
						stepval = 4096
					else
