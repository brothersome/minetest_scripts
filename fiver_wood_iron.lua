if not init then
	...
	mat_generate_energy =  'moreblocks:micro_tree'
	mat_build='default:steel_ingot 10'
	mat_upgrade='default:diamondblock'
	mat1 = 'basic_machines:iron_dust_66'
	mat1_check = 'basic_machines:iron_dust_66 5'
	mat2 ='basic_machines:iron_dust_33'
	mat2_check = 'basic_machines:iron_dust_33 5'
	mat2_max= 'basic_machines:iron_dust_33 90'
	mat3 = 'default:steel_ingot'
	mat4 = 'default:steelblock'
	mat5 = 'basic_machines:iron_dust_00'
	mat5_max = 'basic_machines:iron_dust_00 90'
	...
	if no_wood_cnt <= 0 then
		if index < 1 then
			if index >= 0 then
				...
				if side == 0 then
					if not move.forward() then
						if machine.energy() > 0.1 then
							dig.forward()
						else
							machine.generate_power(mat_generate_energy)
						end
						return
					end
				end
			...
			if check_inventory.right(mat5,'dst') then
				take.right(mat5,'dst')
				craft(mat2)
			end
			if check_inventory.self(mat3,'main') then
				if not check_inventory.right(mat3,'src') and  not check_inventory.self(mat2_max,'main') and not check_inventory.self(mat5max,'main') then
